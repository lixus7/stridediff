...
class Trainer(object):
    ...
    def sample(self, num, size_every, shape=None,
               big_k=None, med_k=None, small_k=None, last_k_always_micro=None,
               tau_energy=None, tau_dlogP=None, tau_pv=None):
        if self.logger is not None:
            tic = time.time()
        ...
        for _ in range(num_cycle):
            sample = self.ema.ema_model.generate_mts(
                ...
                med_k=med_k,
                small_k=small_k,
                last_k_always_micro=last_k_always_micro,
    ...
    def restore(self, raw_dataloader, shape=None, coef=1e-1, stepsize=1e-1, sampling_steps=50,
                ...
                big_k=None, med_k=None, small_k=None, last_k_always_micro=None,
                tau_energy=None, tau_dlogP=None, tau_pv=None,
                band_eta=0.0, use_projection=False, use_langevin=True):
        """Conditional infill / forecast loop over the test dataloader.

        When ``banded`` is True we dispatch to
        :meth:`Diffusion_TS.sample_infill_banded` — the frequency-aware
        dynamic-jump sampler transplanted from stridediff's unconditional
        `sample()` — forwarding the (per-dataset) big_k / med_k / small_k /
        last_k_always_micro / tau_* schedule.  Otherwise we keep the
        original Diffusion-TS dispatch (full-T ``sample_infill`` if
        ``sampling_steps == num_timesteps``, else ``fast_sample_infill``).
        """
        if self.logger is not None:
            tic = time.time()
        ...
        if banded:
            _big_k = int(big_k) if big_k is not None else 30
            _med_k = int(med_k) if med_k is not None else 20
            _small_k = int(small_k) if small_k is not None else 1
            _last_micro = int(last_k_always_micro) if last_k_always_micro is not None else 12
            _tau_energy = float(tau_energy) if tau_energy is not None else 0.5
            _tau_dlogP = float(tau_dlogP) if tau_dlogP is not None else 0.01
            _tau_pv = float(tau_pv) if tau_pv is not None else 0.08
            if self.logger is not None:
                self.logger.log_info(
                    f'[banded] big_k={_big_k} med_k={_med_k} small_k={_small_k} '
                    f'last_k_always_micro={_last_micro} '
                    f'tau_energy={_tau_energy} tau_dlogP={_tau_dlogP} tau_pv={_tau_pv} '
                    f'eta={band_eta} use_projection={use_projection} '
                    f'use_langevin={use_langevin}'
                )

        for idx, (x, t_m) in enumerate(raw_dataloader):
            x, t_m = x.to(self.device), t_m.to(self.device)
            if banded:
                sample = self.ema.ema_model.sample_infill_banded(
                    shape=x.shape, target=x * t_m, partial_mask=t_m,
                    model_kwargs=model_kwargs,
                    big_k=_big_k, med_k=_med_k, small_k=_small_k,
                    last_k_always_micro=_last_micro,
                    tau_energy=_tau_energy, tau_dlogP=_tau_dlogP, tau_pv=_tau_pv,
                    eta=band_eta,
                    use_projection=use_projection,
                    use_langevin=use_langevin,
                )
            elif sampling_steps == self.model.num_timesteps:
